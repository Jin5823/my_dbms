...
int init_buffer_hash_lru(int buf_num){
	...
	for (int i = 0; i < (buf_num-1); i++){
		...
		pthread_mutex_init(&mem_buff->page_latch, NULL);
	}
...
page_t* buff_read_page_trx(int tid, pagenum_t pagenum) {
	...
	if (page_frame == NULL){
		...
		if (mem_lru.Tail->table_id == 0){
			...
			pthread_mutex_lock(&page_frame->page_latch);
			...
			/*while (page_frame->is_pinned != 0){
				pthread_mutex_lock(&page_frame->prev_lru->page_latch);
				pthread_mutex_unlock(&page_frame->page_latch);
...
void dirty_mark_trx(int tid, pagenum_t pagenum){
	pthread_mutex_lock(&buffer_latch);
