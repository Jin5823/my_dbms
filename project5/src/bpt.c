...
pagenum_t read_header_trx(int tid){
	void *ptr;
	pagenum_t a;
	ptr = buff_read_page_trx(tid, 0);
	a = ((HeaderPage*)ptr)->RootPageNumber;
	unlock_page(tid, 0);
	return a;
}
...
pagenum_t find_leaf_trx(int tid, pagenum_t rootnum, int64_t key) {
	...
	pagenum_t temp;
	void *ptr;
	ptr = buff_read_page_trx(tid, cnum);
	...
	while (((LeafPage*)ptr)->IsLeaf == 0) {
		...
		if ( i==-1 ){
			...
			temp = ((InternalPage*)ptr)->OneMorePN;
			unlock_page(tid, cnum);
			cnum = temp;
			ptr = buff_read_page_trx(tid, cnum);
		}else{

			temp = ((InternalPage*)ptr)->InternalKeyPN[i].PageNumber;
			unlock_page(tid, cnum);
			cnum = temp;
			ptr = buff_read_page_trx(tid, cnum);
		}
...
KeyValue * find_trx(int tid, pagenum_t rootnum, int64_t key ) {
	...
	LeafPage_num re;
	void *ptr;
	ptr = buff_read_page_trx(tid, rootnum);
	re.leaf = ptr;
	re.pagenum = rootnum;
...
KeyValue * update_trx(int tid, pagenum_t rootnum, int64_t key, char * values){
	...
	if (i == re.leaf->NumberOfKeys){
    	return NULL;
    	// 해당 key를 끝까지 다 봤는데 없을 경우
    }else{
    	void * ptr;
    	ptr = &(re.leaf->LeafKeyValue[i]);
		strcpy(re.leaf->LeafKeyValue[i].Value, values);
		return (KeyValue *)ptr;
    	// 찾아서 수정
    }
}
